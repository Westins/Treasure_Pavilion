						...
						<table class="table table-bordered table-hover">
							<thead>
								<tr>
									...
									<td class="text-left">店铺介绍</td>
									<td class="text-right">销量</td>
									<td class="text-right">操作</td>
								</tr>
							</thead>
							<tbody>
								<c:forEach items="${UserShopsByUserList }"
									var="UserShopsByUserList">
									<c:forEach items="${UserShopsByUserList.shops }"
										var="ShopsList">
										<tr>
											<td class="text-center"><a href="${pageContext.request.contextPath }/Main/ShowGoods.do?shopsId=${ShopsList.shopsId}"><img
													width="140px" height="70px"
													src="${pageContext.request.contextPath }/${ShopsList.shopsLogo}"
													alt="Aspire Ultrabook Laptop"
													title="Aspire Ultrabook Laptop"> </a></td>
											<td class="text-left"><a href="${pageContext.request.contextPath }/Main/ShowGoods.do?shopsId=${ShopsList.shopsId}">${ShopsList.shopsName}</a></td>
											<td class="text-right"><c:if test="${ShopsList.shopState==1}">正在营业</c:if></td>
											<td class="text-right">${ShopsList.shopsSalesVolume}</td>
											<td class="text-right">
												<a class="btn btn-danger" title="" data-toggle="tooltip"
												onclick="deleteUserShops('${UserShopsByUserList.userShopsId}')" data-original-title="移除"><i
...
============================================ -->
	...
	<script type="text/javascript">
	function deleteUserShops(userShopsId) {
		$.ajax({
			    url:"${pageContext.request.contextPath }/Main/deleteUserShops.do",//请求的url地址
			   // dataType:"json",   返回格式为json
			    async:true,//请求是否异步，默认为异步，这也是ajax重要特性
			    data:{"userShopsId":userShopsId},    //参数值
			    type:"POST",   //请求方式
			    beforeSend:function(){
				//请求前
			    },
